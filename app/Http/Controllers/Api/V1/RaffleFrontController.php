...
class RaffleFrontController extends ApiController
{
    ...
    public function filterByCategory(Request $request)
    {
        ...
        if ($request->get('category') == 'Todos')
            $raffles = Raffle::where('progress', '<', 100)->orderBy('progress', 'DESC')
                ->when($countries, function ($query) use ($countries) {
                    return $query->whereIn('location',$countries);
                })
                ->paginate(10);
        else
            $raffles = $this->raffleRepository->getRafflesByCategory($request->get('category'),null,$request->get('countries'));
    ...
    public function filterByPercent(Request $request)
    {
        ...
        if ($request->get('category') == 'Todos')
            ...
            $raffles = $this->raffleRepository->getRafflesByCategory($request->get('category'),$request->get('criteria'),$request->get('countries'));
        foreach ($raffles as $raffle) {
            $response .= '<div class="row padding20 bg-rifas1 center-block '.$raffle->id.'">'.PHP_EOL;
            $response .= '<div class="col-xs-4 col-md-6" style="padding-left: 23px;padding-right: 0">'.PHP_EOL;
            $response .= '<div class="hidden-lg visible-xs padding-top-20 padding-left-0">'.PHP_EOL;
    ...
    public function filterByCountry(Request $request)
    {
        ...
        if ($request->get('category') == 'Todos') {
            ...
            $raffles = $this->raffleRepository->getRafflesByCategory($request->get('category'),$request->get('criteria'),$countries);
        foreach ($raffles as $raffle) {
            $response .= '<div class="row padding20 bg-rifas1 center-block '.$raffle->id.'">'.PHP_EOL;
            $response .= '<div class="col-xs-4 col-md-6" style="padding-left: 23px;padding-right: 0">'.PHP_EOL;
            $response .= '<div class="hidden-lg visible-xs padding-top-20 padding-left-0">'.PHP_EOL;
